...
.. _info_auto_not_exact:
      ...
      .. example:: `info_auto` output that isn't a valid proof
         ...
         order.  Another would be to add parentheses to the `do 2`,
         which gives a different result.  The fixes are shown inline
         in the example.
...
- For hints other than :cmd:`Hint Extern`: If the database is discriminated, only
  ...
  both the goal and the pattern are identical.  Marking definitions as Hint Opaque
  reduces the number of matches, but it may cause some proof searches that previously
  worked to fail.
...
- For `Extern` hints: If the hint has a pattern, hints matching the goal will be tried.
  In this case, matching uses a form of `syntactic unification
...
(subject to :ref:`locality attributes<controlling-locality-of-commands>`) as
...
--------------
   ...
   *(Deprecated since version 8.10:* If no :token:`ident`\s
   ...
   Rewrites with theorems such as :n:`forall n m, n+m=m+n` or
   :n:`forall n m p, (n+m)+p=n+(m+p)` (eg :n:`Hint Extern ... => rewrite ...`)
   should be used only in :cmd:`Hint Immediate` to prevent them from wastefully
   being applied .
   ...
   .. cmd:: Hint Resolve {+ {| @qualid | @one_term } } {? @hint_info } {? : {+ @ident } }
      ...
      type of :n:`@qualid` to the specified hint databases (:n:`@ident`\s). If specified,
      :n:`@natural` is the cost of the hint; otherwise the cost of the hint is the number of
      subgoals generated by :tacn:`simple apply`. The
      ...
      :n:`@qualid` to the hint list.  If the type reduces to a type starting with a product,
      the command also adds :tacn:`simple apply` :n:`@qualid` to the hints list.
      Note that tactics printed in debug output are similar to but not exactly the same as the
      ones executed by proof search.
   ...
   .. cmd:: Hint {| Transparent | Opaque } {+ @qualid } {? : {+ @ident } }
      ...
      the goal unifies with the theorem used in a :cmd:`Hint Resolve` hint.
      Currently the head constant is always treated as opaque
      (see :ref:`example <head_const_opaque>`).  See
...
.. _head_const_opaque:
      ...
      .. example:: Head Constants are always opaque
         ...
         Adding :n:`Hint Unfold Tru : db` will make `auto` succeed.  (If the goal
         was :n:`True = Tru`, the head constant is :n:`eq` and :n:`Tru` will be unfolded.)
   ...
   .. cmd:: Hint {| Constants | Projections | Variables } {| Transparent | Opaque } {? : {+ @ident } }
      ...
      Sets the default transparency for constants (from :cmd:`Definition`\s and :cmd:`Example`\s),
      projections (from :cmd:`Record`\s and :cmd:`Structure`\s)
      or variables (from :cmd:`Hypotheses` and :cmd:`Variables`) for
