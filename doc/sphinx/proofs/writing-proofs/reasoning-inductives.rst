...
.. opt:: All Depth @natural
   ...
   To prevent this, `All depth` controls to which depth should the `All` predicate be defined.
   For instance, if `All_depth = 1`, the `All` predicate for `prod` will be
   generated, `prod_all`, but the `All` predicate for `prod_all` will not be generated.
   It is default value is `0`
...
.. cmd:: Scheme All for @reference {? with {+, @ident } }
   ...
   To provide better eliminators, the `with` clause enables to generate partial
...
.. example:: Nesting With Prod
   ...
   For instance, if generate an eliminator for `LeftTree` after the generation
   of `prod_all_10`, it will be used instead of `prod_all`.
