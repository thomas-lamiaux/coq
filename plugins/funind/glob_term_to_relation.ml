...
let rocq_True_ref () = Rocqlib.lib_ref "core.True.type"
let rocq_False_ref () = Rocqlib.lib_ref "core.False.type"
...
let make_discr_match_brl i =
  List.map_i
    (fun j {CAst.v = idl, patl, _} ->
      CAst.make
      @@
      if Int.equal j i then (idl, patl, mkGRef (rocq_True_ref()))
      else (idl, patl, mkGRef (rocq_False_ref())))
    0
