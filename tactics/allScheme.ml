...
let add_suffix v suffix = Id.of_string @@ Id.to_string v ^ suffix
...
(** Check a list of boolean is included in an other for [false <= true] *)
let rec inclusion_bool_list l l' =
  match l, l' with
  | [], [] -> true
  | false :: l, _ :: l' -> inclusion_bool_list l l'
  | true :: l, true :: l' -> inclusion_bool_list l l'
  | _, _ -> false
...
let compute_positive_uparams_and_suffix env kn mib user_strpos =
  let default_strpos = compute_params_rec_strpos env kn mib in
  match user_strpos with
  | None -> (default_strpos, fst default_suffix, snd default_suffix)
  | Some user_strpos ->
      if inclusion_bool_list user_strpos default_strpos
      then let partial_suffix = partial_suffix user_strpos in
           (user_strpos, fst partial_suffix, snd partial_suffix)
      else CErrors.user_err Pp.(
          hov 0 (
               strbrk "The user-given uniform parameters allowed to nested on "
            ++ str "[" ++ prlist_with_sep (fun () -> str ", ") bool user_strpos ++ str "]"
            ++ strbrk " are not included in the ones that can acutally be nested on "
            ++ str "[" ++ prlist_with_sep (fun () -> str ", ") bool default_strpos ++ str "]"
          ))

(** Warning for looking up the [all] predicate and its theorem  *)
let warn_lookup_not_found =
  CWarnings.create ~name:("register_all") ~category:Deprecation.Version.v9_2
...
let lookup_all ind ind_nested args_are_nested =
    ...
    | Some ref_pred -> Some (true, ref_pred)
    | None ->
...
let lookup_all_theorem ind ind_nested args_are_nested =
  ...
  | Some ref_pred ->
      match DeclareScheme.lookup_scheme_opt  thm ind_nested with
      | Some ref_thm -> Some (true, ref_pred, ref_thm)
      | None -> warn_lookup_not_found (thm, ind,ind_nested);
...
let instantiate_all_uparams partial_nesting strpos inst_uparams inst_preds =
  ...
  let inst_sparam =
    List.fold_right (fun (positive, inst_uparam, pred) acc ->
      ...
      | None -> if partial_nesting
                then inst_uparam :: acc
                else inst_uparam :: (snd @@ mkFunUnit inst_uparam s) :: acc
      )
  ...
  let inst_lfth =
    List.fold_right (fun (positive, inst_uparam, (pred, pred_hold)) acc ->
      ...
      | None, None ->
        if partial_nesting
        then inst_uparam :: acc
        else inst_uparam :: (snd @@ mkFunUnit inst_uparam s) :: (snd @@ mkFuntt inst_uparam s) :: acc
      | _, _ -> assert false
    )
...
let view_argument kn mib key_uparams strpos t : argument State.t =
...
let fresh_sort_ql i =
    let* sigma = get_sigma in
    let sigma, q, u = Evd.new_sort_info ~sort_rigid:true UnivRigid sigma in
...
let create_fresh_sorts_ql strpos =
  ...
  list_mapi (fun i _ -> fresh_sort_ql i) init
...
let compute_one_return_sort mib ind is_nested u sub_temp fresh_sorts_ql =
  ...
  let sort_if_nested is_nested make_sort u =
    ...
    if not is_nested then
      ...
      let (sigma, l_alg) = Evd.new_univ_level_variable UnivRigid sigma in
      let u_alg = Univ.Universe.make l_alg in
      let u_return_sort = Univ.Universe.sup new_u u_alg in
      (fun s -> return (Some (mkSort u_alg), mkSort u_return_sort) (update_sigma s sigma))
    end
  in
  ...
  let rec aux i key_uparams key_preds key_uparams_preds key_preds_hold tel cc =
    ...
    | decl :: tel ->
        ...
        if (Option.has_some @@ get_value decl) then
          ...
          (* create (PA : A -> s) *)
          let pred_id = Name.map_id (fun id -> Id.of_string @@ "P" ^ Id.to_string id) (get_name decl) in
          ...
          if not mk_pred_hold then
            ...
            let pred_hold_id = Name.map_id (fun id -> Id.of_string @@ "HP" ^ Id.to_string id) (get_name decl) in
...
let rec make_rec_call_hyp kn pos_ind mib rep_inds ((key_uparams, key_preds, key_uparams_preds) as key_up) strpos ualg key_arg arg_type =
  ...
  | ArgIsNested (kn_nested, pos_nested, mib_nested, mib_nested_strpos, ind_nested,
    ...
    begin
      ...
      let compute_pred i x b = compute_pred b (
...
let generate_all_aux suffix kn u sub_temp mib uparams strpos nuparams : mutual_inductive_entry t =
  ...
  dbg Pp.(fun () -> str "Type Sparse Parametricity = "
          ++ (print_constr_env env sigma (it_mkProd_or_LetIn ind_ty params)) ++ str "\n");
  List.iteri (fun i ty_cst -> dbg Pp.(fun () ->
      str "TY CST_" ++ int i ++ str " = " ++
        print_constr_env env sigma (it_mkProd_or_LetIn ty_cst params) ++ str "\n")) ctors_ty;
  let* sigma = get_sigma in
  let uv = Evd.ustate sigma in
  dbg Pp.(fun () -> str "Ustate.T = " ++ UState.pr uv ++ str "\n");
  (* RETURN *)
  return mie
...
let generate_all_predicate env sigma kn u mib strpos suffix =
  ...
  let (sigma, mie) = run env sigma @@ generate_all_aux suffix kn u sub_temp mib uparams strpos nuparams in
  (sigma, mie)
...
let fresh_inductive_instance ind =
  let* env = get_env in
  let* sigma = get_sigma in
  let sigma, ((_, u)) = Evd.fresh_inductive_instance env sigma ind in
  (fun s -> return u (update_sigma s sigma))
...
let rec make_rec_call_proof kn knu pos_ind mib ((key_uparams, _, _) as key_up) key_preds_hold key_fixs strpos key_arg arg_type =
  ...
  | ArgIsNested (kn_nested, pos_nested, mib_nested, mib_nested_strpos, ind_nested,
    ...
    begin
      ...
      let compute_pred_preds i x b = compute_pred b (make_rec_call_hyp kn pos_ind mib (IndIsKn knu) key_up strpos None) i x in
      let* rec_preds = array_map2i compute_pred_preds inst_uparams (Array.of_list mib_nested_strpos) in
      let compute_pred_holds i x b = compute_pred b (make_rec_call_proof kn knu pos_ind mib key_up key_preds_hold key_fixs strpos) i x in
...
let generate_all_theorem_aux kn kn_nested focus u mib uparams strpos nuparams : constr t =
  ...
  let@ (key_fixs, pos_ind, ind) =
    ...
    else fun cc -> cc ([], 0, mib.mind_packets.(0))
  in
...
let generate_all_theorem env sigma kn kn_nested focus u mib strpos =
  ...
  dbg Pp.(fun () -> str "All Theorem = " ++ Termops.Internal.print_constr_env env sigma tm ++ str "\n");
  dbg Pp.(fun () -> str "UState = " ++ UState.pr (Evd.ustate sigma) ++ str "\n");
  dbg Pp.(fun () -> str "---------------------------------------- \n\n");
  (sigma, tm)
