      ...
      let i =
          ...
          | Not_found ->
            ...
            hov 0 (
                  strbrk "The variable "
              ++ Name.print name
              ++ strbrk " is not included in the uniform-parameters that"
              ++ strbrk " are strictly positive, and can be nested on" ++
              if List.is_empty allowed_uparams then
                strbrk " as they are none."
            else
              str ": " ++ prlist_with_sep (fun () -> str ", ") Name.print allowed_uparams
            ))
      in
...
let generate_all_aux suffix kn u sub_temp mib uparams strpos nuparams : mutual_inductive_entry t =
  ...
  List.iteri (fun i ty_cst -> dbg Pp.(fun () ->
         str "Type Constructor " ++ int i ++ str " = "
      ++ print_constr_env env sigma (it_mkProd_or_LetIn ty_cst params)
      ++ fnl ())) ctors_ty;
  let* sigma = get_sigma in
  dbg Pp.(fun () -> str "Ustate.t = " ++ UState.pr (Evd.ustate sigma) ++ str "\n");
  (* RETURN *)
  return mie
