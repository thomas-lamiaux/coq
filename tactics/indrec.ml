...
let get_params_sep sigma mib u =
  let (sigma, up_params) = paramdecls_fresh_template sigma (mib, u) in
  let (uparams, nuparams) = Context.Rel.chop_nhyps mib.mind_nparams_rec (List.rev up_params) in
...
let rec make_rec_call_ty kn pos_ind mib ind_bodies key_preds key_arg ty : (ERelevance.t * constr) option t =
  let* (loc, v) = view_arg kn mib ty in
  (* inst argument *)
  ...
  | ArgIsNested (kn_nested, pos_nested, mib_nested, mib_nested_strpos, ind_nested,
    ...
    begin
      ...
      let uparams_nested = of_rel_context @@ List.rev @@ fst @@
      ...
      let compute_pred i x b = compute_pred b (fun a b -> State.map (fun x -> Option.map snd x)
        @@ make_rec_call_ty kn pos_ind mib ind_bodies key_preds a b) i x in
      ...
      if Array.for_all Option.is_empty rec_preds then
        ...
        (* Instantiation *)
        let ind_args = Array.concat [inst_uparams; inst_nuparams_indices; [|inst_arg|]] in
        let* rec_hyp = typing_checked_appvect ref_ind ind_args  in
...
let make_rec_call_cc rec_hyp kn pos_ind mib ind_bodies key_preds _ key_arg cc =
  ...
  if rec_hyp then begin
    let* rec_call = make_rec_call_ty kn pos_ind mib ind_bodies key_preds key_arg arg in
    match rec_call with
...
let closure_ctors rec_hyp kn mib u ind_bodies binder key_uparams nuparams key_nuparams_opt key_preds =
  fold_right_state (fun a l -> a :: l) ind_bodies (
    ...
    iterate_ctors mib ind u (
      ...
      let* fct = make_type_ctor kn u mib ind_bodies pos_list (pos_ind, ind, dep, sort)
...
let gen_elim_type print_constr rec_hyp kn u mib uparams nuparams (ind_bodies : elim_info list) (focus : int) =
...
let rec make_rec_call kn pos_ind mib ind_bodies key_preds key_fixs key_arg ty : (constr option) State.t =
  let* (loc, v) = view_arg kn mib ty in
    (* inst argument *)
  ...
  | ArgIsNested (kn_nested, pos_nested, mib_nested, mib_nested_strpos, ind_nested,
      ...
      begin
        ...
        let compute_pred_preds i x b = compute_pred b (fun a b -> State.map (fun x -> Option.map snd x)
              @@ make_rec_call_ty kn pos_ind mib ind_bodies key_preds a b) i x in
        let* rec_preds = array_map2i compute_pred_preds inst_uparams (Array.of_list mib_nested_strpos) in
        let compute_pred_holds i x b = compute_pred b (make_rec_call kn pos_ind mib ind_bodies key_preds key_fixs) i x in
        ...
        (* Instantiation *)
        let ind_args = Array.concat [inst_uparams; inst_nuparams_indices; [|inst_arg|] ] in
        let* rec_hyp = typing_checked_appvect fth ind_args in
        return @@ Some (rec_hyp)
        end
      end
  | _ -> return None
...
let compute_args_fix rec_hyp kn pos_ind mib ind_bodies pos_list key_preds key_fixs key_args =
  CList.fold_right_i (fun pos_arg key_arg acc ->
    ...
    if rec_hyp then
      ...
      let* rec_call = make_rec_call kn pos_ind mib ind_bodies key_preds key_fixs key_arg ty_arg in
      match rec_call with
...
let gen_elim_term print_constr rec_hyp kn u mib uparams nuparams (ind_bodies : elim_info list) (focus : int) =
